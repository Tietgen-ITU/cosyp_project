...
def count_words(cur, seed=None):
    ...
    sorted_article_freqs = sorted(word_article_freqs.items(), key=lambda x: x[1], reverse=True)
...
def generate_frequency_queries(cur, num_queries=10, max_articles_sourced=1000, num_words=(1, 6), seed=None):
    word_freqs = load_word_freqs("word_freqs.json")
    print()
    ...
    while len(search_term_score) < 100 * num_queries:
        ...
        lower_terms = set(term for term in search_term.lower().split() if term not in STOP_WORDS)
        # with_score = [(term, word_freqs.get(term, 0)) for term in lower_terms]
        with_article_score = [(term, word_article_freqs.get(term, 0)) for term in lower_terms]
    ...
    sorted_search_terms = sorted(search_term_score.values(), key=lambda x: x[1], reverse=True)
    ...
    return {
        'in_many_articles': list(high_cardinality),
        'in_few_articles': list(low_cardinality),
    }
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "count":
        ...
        print(json.dumps(generate_frequency_queries(pg, seed=1337, num_words=(4, 4)), indent=2))
    else:
        print("idk bro")
    pg.close()
